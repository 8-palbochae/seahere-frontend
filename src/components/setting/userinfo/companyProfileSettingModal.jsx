...
const CompanyProfileSettingModal = ({ onClose, onUpdateProfileImage }) => {
    ...
    const handleFileChange = async (event) => {
        ...
        if (file) {
            ...
            try {
                const response = await axiosInstance.patch(`${url}/s3/profile/company/upload`, formData, {
                    headers: {
    ...
    const handleDefaultImageClick = async () => {
        try {
            await axiosInstance.delete(`${url}/s3/profile/company/delete`);
...
CompanyProfileSettingModal.propTypes = {
    onClose: PropTypes.func.isRequired,
    onUpdateProfileImage: PropTypes.func.isRequired,
};

export default CompanyProfileSettingModal;
