...
const InventoryItemDetails = ({ detailData, productImg }) => {
	...
	return (
		...
		>
			<div className="flex">
				<div
					className={`flex flex-col justify-center transition-transform duration-300 ${
						isSlid ? "translate-x-[-140px]" : "translate-x-0"
					}`}
				...
				<div
					className={`absolute top-0 right-0 h-full flex flex-row bg-white border-l border-gray-200 transition-transform duration-300 ${
						isSlid ? "translate-x-0" : "translate-x-full"
					}`}
					style={{ width: "100px" }}
				>
					<button
						className="flex-1 flex items-center justify-center p-0 bg-gray-400"
						onClick={openModal}
					>
						<img
							src={inventoryIcon.editIcon}
